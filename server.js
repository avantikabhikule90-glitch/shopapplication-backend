...
app.post('/orders/place', async (req, res) => {
  try {
    ...
    if (!user) return res.status(400).json({ success: false, message: 'User not found' });
    let discount = 0;
    if (pointsUsed > 0 && user.points >= pointsUsed && pointsUsed <= 100) {
      discount = Math.floor(total * pointsUsed / 100);
      user.points -= pointsUsed;
    }
    const finalTotal = total - discount;
    const pointsEarned = Math.floor(finalTotal * 0.1);
    await user.save();
    const order = await Order.create({
      userEmail: email, items, total, pointsUsed, discount,
      finalTotal, pointsEarned, status: 'Pending', pointsAwarded: false
    });
    res.json({ success: true, orderId: order._id, pointsEarned, totalPoints: user.points, discount });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
...
// Route 9: Get All Orders (Admin only)
app.get('/orders/all', async (req, res) => {
  try {
    const orders = await Order.find().sort({ createdAt: -1 });
    res.json({ success: true, orders });
...
// Route 10: Update Order Status (Admin) — awards points on Delivered
app.post('/orders/update-status', async (req, res) => {
  try {
    const { orderId, status } = req.body;
    ...
    // Award points ONLY when status becomes Delivered and not already awarded
    if (status === 'Completed' && !order.pointsAwarded) {
      const user = await User.findOne({ email: order.userEmail });
      if (user) {
        const pointsToAward = Math.floor(order.finalTotal * 0.1);
        user.points += pointsToAward;
        await user.save();
        order.pointsAwarded = true;
        console.log('Points awarded: ' + pointsToAward + ' to ' + order.userEmail);
      }
    }
...
// Route 12: Reset All Data (Admin Only)
